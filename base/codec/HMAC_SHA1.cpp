...
void CHMAC_SHA1::HMAC_SHA1(BYTE *text, int text_len, BYTE *key, int key_len, BYTE *digest)
{
	...
	{
			CSHA1::Reset();
			CSHA1::Update((UINT_8 *)key, (unsigned int)key_len);
			CSHA1::Final();
	...
	/* STEP 4 */
	CSHA1::Reset();
	CSHA1::Update((UINT_8 *)AppendBuf1, (unsigned int)(sizeof(m_ipad) + text_len));
	CSHA1::Final();
	...
	/*STEP 7 */
	CSHA1::Reset();
	CSHA1::Update((UINT_8 *)AppendBuf2, (unsigned int)(sizeof(m_opad) + SHA1_DIGEST_LENGTH));
	CSHA1::Final();
