...
public:
    //GLOG配置：程序名，日志路径
    GLogHelper(char* program, string log_path);
    //GLOG内存清理：
    ~GLogHelper();
};

//GLOG配置：
GLogHelper::GLogHelper(char* program,string log_path)
{
    //日志路径
    string LOGDIR = log_path + string(program);
